...
export async function StudentView({
  ...
  me: Student;
  /** 관리자 미리보기. 버튼을 눌러도 동작하지 않도록 정적 표시로 바꾼다. */
  readOnly?: boolean;
}) {
      ...
      <PageShell>
        {readOnly ? (
          <div className="flex items-center justify-between gap-3 py-1">
            <h1 className="text-lg font-semibold">{me.name} 님의 화면</h1>
            <a href="/admin/students" className="text-xs text-neutral-500">
              ← 학생 관리
            </a>
          </div>
        ) : (
        ...
        {isCipDay(wd) && !readOnly ? (
          ...
          <section className="rounded-2xl bg-surface p-5">
            ...
            <ul className="mt-2 space-y-1.5">
              {cipStates.map((st) => (
                <li key={st.session} className="flex items-center justify-between gap-2 text-sm">
                  <span className="text-neutral-400">{SESSION_LABEL[st.session]}</span>
                  <span className="truncate text-xs text-neutral-500">{st.reason ?? ""}</span>
                  <span className={`shrink-0 rounded-md border px-2 py-0.5 text-xs ${NS_STYLE[st.status]}`}>
                    {NS_LABEL[st.status]}
                  </span>
                </li>
              ))}
